...
class TfIdfCalculator:
    ...
    def calculate_tf_idf(self, num_tweets=1, min_df=3,max_df=0.85):
        ...
        tf_idf_transformer = TfidfTransformer(sublinear_tf=True,norm="l2")#Modifed parameter because values have been higher than one
    ...
    def nth_simiular_tweets(self,num_tweets = 1, min_df=5, n = 5):
        ...
        top_score_tweets = []
        for i in range(n):
            top_score_tweets.append([top_n_values[i],self.corpus[multi_indices[i][0]],self.corpus[multi_indices[i][1]]])
